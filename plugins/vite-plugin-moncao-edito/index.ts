...
export function getWorks(options: IMonacoEditorOpts) {
  let works: IWorkerDefinition[] = options.languageWorkers?.map(
    (work) => languageWorksByLabel(options.monacoPackage ?? "monaco-editor")[work]
  ) ?? [];
...
export interface IMonacoEditorOpts {
   ...
   *
   */
  monacoPackage?: string
}

export default function monacoEditorPlugin(options: IMonacoEditorOpts): Plugin {
  const monacoPackage = options.monacoPackage || 'monaco-editor'
  const languageWorkers =
    options.languageWorkers || (Object.keys(languageWorksByLabel(monacoPackage)) as EditorLanguageWorks[]);
    ...
    writeBundle() {
      ...
      const distPath = options.customDistPath
        ...
        : path.join(
            ...
            resolvedConfig.build.outDir,
            resolvedConfig.base,
            options.publicPath ?? 'monacoeditorwork'
          );
